...
function gunwipall
  set --local _commit=(git log --grep='--wip--' --invert-grep --max-count=1 --format=format:%H)
...
# function gbds
#   set --local default_branch (git_main_branch)
#   test $status -eq 0 || set default_branch (git_develop_branch)

#   git for-each-ref refs/heads/ "--format=%(refname:short)" | \
#     while read branch
#       set --local merge_base=(git merge-base $default_branch $branch)
...
#   set --local repo="${${@[(r)(ssh://*|git://*|ftp(s)#://*|http(s)#://*|*@*)(.git/#)#]}:-$_}"

#   # clone repository and exit if it fails
#   command git clone --recurse-submodules $argv || return
...
# function ggu
#   test (count $argv) -ne 1 && set --local b (git_current_branch)
...
# function ggl
#   if test (count $argv) -ne 0 && test (count $argv) -ne 1
#     git pull origin "$argv"
#   else
#     test (count $argv) -eq 0 && set --local b (git_current_branch)
#     git pull origin "${b:=$1}"
#   end
# end
